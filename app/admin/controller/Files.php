...
class Files extends Base
{
    ...
    {
        parent::initialize();
        $this->mod = new \app\common\model\Files;
    }
    public function index($keyword = '', $page = 1, $isuse = 1)
    {
        ...
        $files_list = $this->mod->where($map)->order(['id' => 'DESC'])->paginate(30, false, ['page' => $page]);
    ...
    public function details($id)
    {
        ...
        $this->success('获取成功', $this->mod::find($id));
    }
    //获取文件列表
    public function getjson($ftype = 0, $page = 1, $keyword = '')
    {
        ...
        //只读取未使用的附件
        $map[] = ['ypcms_id', '=', 0];
        $files_list = $this->mod->where($map)->order(['id' => 'DESC'])->paginate(24, false, ['page' => $page]);
    ...
    public function clearfiles(){
        $map=[];
        $map[]=['category_id|ypcms_id','=','0'];
        $map[]=['ypcms_type','=',''];
        $this->mod::where($map)->delete();
        $this->success("清理成功");
    }
}
